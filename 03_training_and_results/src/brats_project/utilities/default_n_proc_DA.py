...
def get_allowed_n_proc_DA():
    ...
    IMPORTANT: if the environment variable BRATS_N_PROC_DA is set it will overwrite anything in this script
    (see first line).
    ...
    if "BRATS_N_PROC_DA" in os.environ.keys():
        use_this = int(os.environ["BRATS_N_PROC_DA"])
    elif "nnUNet_n_proc_DA" in os.environ.keys():
    ...
    cpu_count = os.cpu_count() or use_this
    use_this = min(use_this, cpu_count)
    return use_this
