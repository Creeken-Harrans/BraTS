...
from brats_project.configuration import ANISO_THRESHOLD
from brats_project.experiment_planning.experiment_planners.network_topology import get_pool_and_conv_props
...
from brats_project.preprocessing.normalization.map_channel_name_to_normalization import get_normalization_scheme
from brats_project.preprocessing.resampling.default_resampling import resample_data_or_seg_to_shape, compute_new_shape
...
class ExperimentPlanner(object):
    def __init__(self, dataset_name_or_id: Union[str, int],
                 ...
                 preprocessor_name: str = 'DefaultPreprocessor', plans_name: str = 'ProjectPlans',
        ...
        self.raw_dataset_folder = join(PROJECT_RAW, self.dataset_name)
        preprocessed_folder = join(PROJECT_PREPROCESSED, self.dataset_name)
        ...
        if not isfile(join(preprocessed_folder, 'dataset_fingerprint.json')):
            raise RuntimeError('Fingerprint missing for this dataset. Please run the project fingerprint extraction step first')
    ...
    def plan_experiment(self):
        ...
        shutil.copy(join(self.raw_dataset_folder, 'dataset.json'),
                    join(PROJECT_PREPROCESSED, self.dataset_name, 'dataset.json'))
    ...
    def save_plans(self, plans):
        ...
        plans_file = join(PROJECT_PREPROCESSED, self.dataset_name, self.plans_identifier + '.json')
        ...
        maybe_mkdir_p(join(PROJECT_PREPROCESSED, self.dataset_name))
        save_json(plans, plans_file, sort_keys=False)
        print(f"Plans were saved to {join(PROJECT_PREPROCESSED, self.dataset_name, self.plans_identifier + '.json')}")
