...
from brats_project.paths import PROJECT_PREPROCESSED
from brats_project.run.load_pretrained_weights import load_pretrained_weights
...
def get_trainer_from_args(dataset_name_or_id: Union[int, str],
    ...
    search_root = join(brats_project.__path__[0], "training")
    nnunet_trainer = recursive_find_python_class(search_root, trainer_name, 'brats_project.training')
    if nnunet_trainer is None:
        raise RuntimeError(
            f'Could not find trainer {trainer_name} in {search_root}. '
            'Place project trainers under src/brats_project/training.'
        )
    assert issubclass(nnunet_trainer, nnUNetTrainer), (
        'The requested trainer class must inherit from nnUNetTrainer'
    )

    # handle dataset input. If it's an ID we need to convert to int from string
    dataset_name_or_id_str = str(dataset_name_or_id)
    if dataset_name_or_id_str.startswith('Dataset'):
        dataset_name_or_id = dataset_name_or_id_str
        pass
    else:
        try:
            dataset_name_or_id = int(dataset_name_or_id_str)
        except ValueError:
            raise ValueError(f'dataset_name_or_id must either be an integer or a valid dataset name with the pattern '
                             f'DatasetXXX_YYY where XXX are the three(!) task ID digits. Your '
                             f'input: {dataset_name_or_id_str}')

    preprocessed_dataset_folder_base = join(PROJECT_PREPROCESSED, maybe_convert_to_dataset_name(dataset_name_or_id))
...
def run_training(dataset_name_or_id: Union[str, int],
                 ...
                 trainer_class_name: str = 'SegTrainer',
                 plans_identifier: str = 'ProjectPlans',
...
def run_training_entry():
    ...
    parser.add_argument('-tr', type=str, required=False, default='SegTrainer',
                        help='[OPTIONAL] Trainer class name. Default: SegTrainer')
    parser.add_argument('-p', type=str, required=False, default='ProjectPlans',
                        help='[OPTIONAL] Plans identifier. Default: ProjectPlans')
    parser.add_argument('-pretrained_weights', type=str, required=False, default=None,
                        help='[OPTIONAL] Path to a pretrained checkpoint file. Will only '
    ...
    parser.add_argument('-device', type=str, default='cuda', required=False,
                        help="Use this to set the device. Available options are 'cuda', 'cpu' and 'mps'. "
