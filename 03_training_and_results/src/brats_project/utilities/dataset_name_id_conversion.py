...
def find_candidate_datasets(dataset_id: int):
    ...
    if PROJECT_PREPROCESSED is not None and isdir(PROJECT_PREPROCESSED):
        candidates_preprocessed = subdirs(
            PROJECT_PREPROCESSED, prefix=startswith, join=False
        )
    else:
    ...
    if PROJECT_RESULTS is not None and isdir(PROJECT_RESULTS):
        candidates_trained_models += subdirs(
            PROJECT_RESULTS, prefix=startswith, join=False
        )
...
def convert_id_to_dataset_name(dataset_id: int):
    ...
    if len(unique_candidates) > 1:
        raise RuntimeError(
            ...
            "following folders:\n%s\n%s\n%s"
            % (dataset_id, PROJECT_RAW, PROJECT_PREPROCESSED, PROJECT_RESULTS)
        )
