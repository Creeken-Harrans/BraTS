...
from brats_project.preprocessing.resampling.utils import (
...
if TYPE_CHECKING:
    ...
    from brats_project.preprocessing.preprocessors.default_preprocessor import (
        DefaultPreprocessor,
    )
...
class ConfigurationManager(object):
    ...
    def preprocessor_class(self) -> Type[DefaultPreprocessor]:
        preprocessor_class = recursive_find_python_class(
            join(brats_project.__path__[0], "preprocessing"),
...
class PlansManager(object):
    ...
    def _internal_resolve_configuration_inheritance(
        self, configuration_name: str, visited: Optional[Tuple[str, ...]] = None
    ) -> dict:
...
if __name__ == "__main__":
    ...
    plans = load_json(
        join(PROJECT_PREPROCESSED, maybe_convert_to_dataset_name(3), "nnUNetPlans.json")
    )
