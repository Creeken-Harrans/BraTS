...
from brats_project.ensembling.ensemble import ensemble_crossvalidations
from brats_project.evaluation.accumulate_cv_results import accumulate_cv_results
from brats_project.evaluation.evaluate_predictions import compute_metrics_on_folder, load_summary_json
from brats_project.paths import PROJECT_PREPROCESSED, PROJECT_RAW, PROJECT_RESULTS
from brats_project.project_layout import load_project_config
from brats_project.postprocessing.remove_connected_components import determine_postprocessing
from brats_project.utilities.file_path_utilities import maybe_convert_to_dataset_name, get_output_folder, \
...
default_trained_models = tuple([
    {'plans': 'ProjectPlans', 'configuration': '2d', 'trainer': 'SegTrainer'},
    {'plans': 'ProjectPlans', 'configuration': '3d_fullres', 'trainer': 'SegTrainer'},
    {'plans': 'ProjectPlans', 'configuration': '3d_lowres', 'trainer': 'SegTrainer'},
    {'plans': 'ProjectPlans', 'configuration': '3d_cascade_fullres', 'trainer': 'SegTrainer'},
])
...
def filter_available_models(model_dict: Union[List[dict], Tuple[dict, ...]], dataset_name_or_id: Union[str, int]):
    ...
    for trained_model in model_dict:
        plans_manager = PlansManager(join(PROJECT_PREPROCESSED, maybe_convert_to_dataset_name(dataset_name_or_id),
        ...
        if trained_model['configuration'] not in plans_manager.available_configurations:
            print(f"Configuration {trained_model['configuration']} not found in plans {trained_model['plans']}.\n"
                  f"Inferred plans file: {join(PROJECT_PREPROCESSED, maybe_convert_to_dataset_name(dataset_name_or_id), trained_model['plans'] + '.json')}.")
            continue
...
def get_available_validation_folds(trained_model_folder: str) -> Tuple[int, ...]:
    available = []
    for entry in listdir(trained_model_folder):
        if not entry.startswith('fold_'):
            continue
        validation_dir = join(trained_model_folder, entry, 'validation')
        if not isdir(validation_dir):
            continue
        try:
            fold_idx = int(entry.split('_', 1)[1])
        except ValueError:
            continue
...
def discover_trained_models_with_validation(dataset_name_or_id: Union[str, int]) -> Tuple[dict, ...]:
    dataset_name = maybe_convert_to_dataset_name(dataset_name_or_id)
    dataset_results_dir = join(PROJECT_RESULTS, dataset_name)
    if not isdir(dataset_results_dir):
        return tuple()

    discovered = []
    for entry in listdir(dataset_results_dir):
        if entry == 'ensembles':
            continue
        full_dir = join(dataset_results_dir, entry)
        if not isdir(full_dir):
            continue
        try:
            trainer, plans, configuration = convert_identifier_to_trainer_plans_config(entry)
        except Exception:
            continue
        if len(get_available_validation_folds(full_dir)) == 0:
            continue
        discovered.append({
            'plans': plans,
...
def generate_inference_command(dataset_name_or_id: Union[int, str], configuration_name: str,
    ...
    predict_command = f'brats_predict -d {dataset_name_or_id} -i {input_folder} -o {output_folder} -f {fold_str} ' \
...
def find_best_configuration(dataset_name_or_id,
    ...
    if allow_ensembling:
        for i in range(len(allowed_trained_models)):
            for j in range(i + 1, len(allowed_trained_models)):
                ...
                compute_metrics_on_folder(join(PROJECT_PREPROCESSED, dataset_name, 'gt_segmentations'),
    ...
    determine_postprocessing(all_results[best_key]['source'], join(PROJECT_PREPROCESSED, dataset_name, 'gt_segmentations'),
    ...
    return_dict = {
        'folds': folds_per_identifier[best_key],
    ...
    save_json(return_dict, join(PROJECT_RESULTS, dataset_name, 'inference_information.json'))  # save this so that we don't have to run this
    ...
    # print it
    print_inference_instructions(return_dict, instructions_file=join(PROJECT_RESULTS, dataset_name, 'inference_instructions.txt'))
    return return_dict
...
def print_inference_instructions(inference_info_dict: dict, instructions_file: str = None):
    ...
    if is_ensemble:
        ...
        _print_and_maybe_write_to_file(f"brats_ensemble -i {output_folder_str} -o {output_ensemble} -np {default_num_processes}")

    _print_and_maybe_write_to_file("\n***Once inference is completed, run postprocessing like this:***\n")
    _print_and_maybe_write_to_file(f"brats_apply_postprocessing -i OUTPUT_FOLDER -o OUTPUT_FOLDER_PP "
...
def find_best_configuration_entry_point():
    dataset_defaults = load_project_config()["dataset"]
    parser = argparse.ArgumentParser()
    parser.add_argument('dataset_name_or_id', type=str, nargs='?', default=dataset_defaults["name"], help='Dataset Name or id')
    parser.add_argument('-p', nargs='+', required=False, default=[dataset_defaults["plans"]],
                        help=f'List of plan identifiers. Default: {dataset_defaults["plans"]}')
    parser.add_argument('-c', nargs='+', required=False, default=[dataset_defaults["default_configuration"]],
                        help=f'List of configurations. Default: [{dataset_defaults["default_configuration"]}]')
    parser.add_argument('-tr', nargs='+', required=False, default=[dataset_defaults["trainer"]],
                        help=f'List of trainers. Default: {dataset_defaults["trainer"]}')
...
def accumulate_crossval_results_entry_point():
    ...
    parser.add_argument('-f', nargs='+', type=int, default=(0, 1, 2, 3, 4),
                        help='Folds to use. Default: 0 1 2 3 4')
    parser.add_argument('-p', type=str, required=False, default='ProjectPlans',
                        help='Plan identifier in which to search for the specified configuration. Default: ProjectPlans')
    parser.add_argument('-tr', type=str, required=False, default='SegTrainer',
                        help='Trainer class. Default: SegTrainer')
    args = parser.parse_args()
    ...
    if args.o is None:
        ...
        if isdir(merged_output_folder) and len(listdir(merged_output_folder)) > 0:
            raise FileExistsError(
                ...
                f"To avoid data loss, this project requires an empty output folder."
            )
