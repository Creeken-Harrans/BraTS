...
from brats_project.training.data_augmentation.compute_initial_patch_size import (
    get_patch_size,
)
...
from brats_project.training.loss.dice import (
    get_tp_fp_fn_tn,
...
class nnUNetTrainer(object):
    def __init__(
        ...
        self.preprocessed_dataset_folder_base = (
            join(PROJECT_PREPROCESSED, self.plans_manager.dataset_name)
            if PROJECT_PREPROCESSED is not None
            else None
        )
    ...
    def _do_i_compile(self):
        ...
        if self.device == torch.device("mps"):
            if "PROJECT_COMPILE" in os.environ.keys() and os.environ[
                "PROJECT_COMPILE"
            ].lower() in ("true", "1", "t"):
                self.print_to_log_file(
    ...
    def plot_network_architecture(self):
        if self._do_i_compile():
            self.print_to_log_file(
                "Unable to plot network architecture: PROJECT_COMPILE is enabled!"
            )
            return
