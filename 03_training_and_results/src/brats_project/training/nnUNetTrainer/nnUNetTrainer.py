...
from brats_project.configuration import ANISO_THRESHOLD, default_num_processes
from brats_project.evaluation.evaluate_predictions import compute_metrics_on_folder
from brats_project.inference.export_prediction import export_prediction_from_logits, resample_and_save
from brats_project.inference.predict_from_raw_data import nnUNetPredictor
from brats_project.inference.sliding_window_prediction import compute_gaussian
from brats_project.paths import PROJECT_PREPROCESSED, PROJECT_RESULTS
from brats_project.training.data_augmentation.compute_initial_patch_size import get_patch_size
from brats_project.training.dataloading.nnunet_dataset import infer_dataset_class
from brats_project.training.dataloading.data_loader import nnUNetDataLoader
from brats_project.training.logging.nnunet_logger import MetaLogger
from brats_project.training.loss.compound_losses import DC_and_CE_loss, DC_and_BCE_loss
from brats_project.training.loss.deep_supervision import DeepSupervisionWrapper
from brats_project.training.loss.dice import get_tp_fp_fn_tn, MemoryEfficientSoftDiceLoss
from brats_project.training.lr_scheduler.polylr import PolyLRScheduler
from brats_project.utilities.collate_outputs import collate_outputs
from brats_project.utilities.crossval_split import generate_crossval_split
...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict,
        ...
        self.preprocessed_dataset_folder_base = join(PROJECT_PREPROCESSED, self.plans_manager.dataset_name) \
            if PROJECT_PREPROCESSED is not None else None
        self.output_folder_base = join(PROJECT_RESULTS, self.plans_manager.dataset_name,
                                       self.__class__.__name__ + '__' + self.plans_manager.plans_name + "__" + configuration) \
            if PROJECT_RESULTS is not None else None
        ...
        self.folder_with_segs_from_previous_stage = \
            join(PROJECT_RESULTS, self.plans_manager.dataset_name,
    ...
    def _do_i_compile(self):
        ...
        if 'PROJECT_COMPILE' not in os.environ.keys():
            return True
        else:
            return os.environ['PROJECT_COMPILE'].lower() in ('true', '1', 't')
    ...
    def plot_network_architecture(self):
        if self._do_i_compile():
            self.print_to_log_file("Unable to plot network architecture: PROJECT_COMPILE is enabled!")
            return
            ...
            for i, k in enumerate(dataset_val.identifiers):
                ...
                if next_stages is not None:
                    for n in next_stages:
                        ...
                        expected_preprocessed_folder = join(PROJECT_PREPROCESSED, self.plans_manager.dataset_name,
