...
def get_trainer_from_args(
    ...
    nnunet_trainer = recursive_find_python_class(
        search_root, trainer_name, "brats_project.training"
    )
    ...
    preprocessed_dataset_folder_base = join(
        PROJECT_PREPROCESSED, maybe_convert_to_dataset_name(dataset_name_or_id)
    )
...
def run_ddp(
    ...
    nnunet_trainer = get_trainer_from_args(
        dataset_name_or_id, configuration, fold, tr, p, effective_continue_training
    )
...
def run_training(
    ...
    if num_gpus > 1:
        ...
        effective_continue_training = (
            continue_training and resume_checkpoint is not None
        )
