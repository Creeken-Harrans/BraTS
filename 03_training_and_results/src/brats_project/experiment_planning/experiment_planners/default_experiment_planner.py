...
from brats_project.experiment_planning.experiment_planners.network_topology import (
    get_pool_and_conv_props,
)
from brats_project.imageio.reader_writer_registry import (
    determine_reader_writer_from_dataset_json,
)
from brats_project.paths import PROJECT_RAW, PROJECT_PREPROCESSED
from brats_project.preprocessing.normalization.map_channel_name_to_normalization import (
    get_normalization_scheme,
)
from brats_project.preprocessing.resampling.default_resampling import (
...
class ExperimentPlanner(object):
    def __init__(
        ...
        if not isfile(join(preprocessed_folder, "dataset_fingerprint.json")):
            raise RuntimeError(
                "Fingerprint missing for this dataset. Please run the project fingerprint extraction step first"
            )
    ...
    def save_plans(self, plans):
        ...
        plans_file = join(
            PROJECT_PREPROCESSED, self.dataset_name, self.plans_identifier + ".json"
        )
        ...
        print(
            f"Plans were saved to {join(PROJECT_PREPROCESSED, self.dataset_name, self.plans_identifier + '.json')}"
        )
