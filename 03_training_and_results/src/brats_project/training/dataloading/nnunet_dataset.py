...
from brats_project.training.dataloading.utils import unpack_dataset
import math
...
class nnUNetDatasetNumpy(nnUNetBaseDataset):
    def load_case(self, identifier):
        ...
        if not isfile(data_npy_file):
            data = np.load(join(self.source_folder, identifier + '.npz'))['data'].copy()
        else:
            data = np.load(data_npy_file)

        seg_npy_file = join(self.source_folder, identifier + '_seg.npy')
        if not isfile(seg_npy_file):
            seg = np.load(join(self.source_folder, identifier + '.npz'))['seg'].copy()
        else:
        ...
        if self.folder_with_segs_from_previous_stage is not None:
            ...
            if isfile(prev_seg_npy_file):
                ...
                seg_prev = np.load(join(self.folder_with_segs_from_previous_stage, identifier + '.npz'))['seg'].copy()
        else:
            seg_prev = None
...
class nnUNetDatasetBlosc2(nnUNetBaseDataset):
    ...
    def save_case(
        ...
        data_path = output_filename_truncated + '.b2nd'
        seg_path = output_filename_truncated + '_seg.b2nd'
        properties_path = output_filename_truncated + '.pkl'

        for path in (data_path, seg_path, properties_path):
            if isfile(path):
                os.remove(path)

        # print(output_filename_truncated, data.shape, seg.shape, blocks, chunks, blocks_seg, chunks_seg, data.dtype, seg.dtype)
        blosc2.asarray(np.ascontiguousarray(data), urlpath=data_path, chunks=chunks,
                       blocks=blocks, cparams=cparams)
        blosc2.asarray(np.ascontiguousarray(seg), urlpath=seg_path, chunks=chunks_seg,
                       blocks=blocks_seg, cparams=cparams)
        write_pickle(properties, properties_path)
