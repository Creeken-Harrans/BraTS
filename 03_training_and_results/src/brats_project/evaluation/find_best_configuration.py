...
from brats_project.postprocessing.remove_connected_components import (
    determine_postprocessing,
)
from brats_project.utilities.file_path_utilities import (
...
def discover_trained_models_with_validation(
    ...
    for entry in listdir(dataset_results_dir):
        ...
        if not isdir(full_dir):
            continue
        try:
            trainer, plans, configuration = convert_identifier_to_trainer_plans_config(
                entry
            )
        except Exception:
            continue
...
def determine_comparable_folds(
    ...
    for trained_model in allowed_trained_models:
        ...
        if len(selected_folds) == 0:
            print(
                f"Skipping {identifier}: no validation results found for requested folds {tuple(requested_folds)}."
            )
            continue
...
def generate_inference_command(
    ...
    predict_command = (
        f"brats_predict -d {dataset_name_or_id} -i {input_folder} -o {output_folder} -f {fold_str} "
...
def find_best_configuration(
    ...
    for m in allowed_trained_models:
        ...
        merged_output_folder = join(
            output_folder,
            f"crossval_results_folds_{folds_tuple_to_string(comparable_folds)}",
        )
    ...
    if allow_ensembling:
        for i in range(len(allowed_trained_models)):
            for j in range(i + 1, len(allowed_trained_models)):
                ...
                identifier = get_ensemble_name(
                    output_folder_1, output_folder_2, comparable_folds
                )

                output_folder_ensemble = join(
                    PROJECT_RESULTS, dataset_name, "ensembles", identifier
                )
                ...
                compute_metrics_on_folder(
                    join(PROJECT_PREPROCESSED, dataset_name, "gt_segmentations"),
                    output_folder_ensemble,
    ...
    save_json(
        return_dict, join(PROJECT_RESULTS, dataset_name, "inference_information.json")
    ...
    print_inference_instructions(
        ...
        instructions_file=join(
            PROJECT_RESULTS, dataset_name, "inference_instructions.txt"
        ),
    )
    return return_dict
...
def print_inference_instructions(
    inference_info_dict: dict, instructions_file: Optional[str] = None
):
    ...
    if is_ensemble:
        ...
        _print_and_maybe_write_to_file(
            f"brats_ensemble -i {output_folder_str} -o {output_ensemble} -np {default_num_processes}"
        )
    ...
    _print_and_maybe_write_to_file(
        f"brats_apply_postprocessing -i OUTPUT_FOLDER -o OUTPUT_FOLDER_PP "
...
def find_best_configuration_entry_point():
    ...
    parser.add_argument(
        ...
        nargs="+",
        required=False,
        default=[dataset_defaults["plans"]],
    ...
    parser.add_argument(
        ...
        nargs="+",
        required=False,
        default=[dataset_defaults["default_configuration"]],
    ...
    parser.add_argument(
        ...
        nargs="+",
        required=False,
        default=[dataset_defaults["trainer"]],
