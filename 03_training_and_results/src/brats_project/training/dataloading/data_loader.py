...
from brats_project.training.dataloading.nnunet_dataset import nnUNetBaseDataset
        ...
        with torch.no_grad():
            with threadpool_limits(limits=1, user_api=None):
                for j, i in enumerate(selected_keys):
                    ...
                    seg_cropped = torch.from_numpy(crop_and_pad_nd(seg, bbox, -1)).to(
                        torch.int16
                    )
                    if seg_prev is not None:
...
if __name__ == "__main__":
    folder = join(PROJECT_PREPROCESSED, "Dataset002_Heart", "nnUNetPlans_3d_fullres")
