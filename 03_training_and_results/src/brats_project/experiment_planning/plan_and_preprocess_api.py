...
from brats_project.experiment_planning.dataset_fingerprint.fingerprint_extractor import DatasetFingerprintExtractor
from brats_project.experiment_planning.experiment_planners.default_experiment_planner import ExperimentPlanner
from brats_project.experiment_planning.verify_dataset_integrity import verify_dataset_integrity
from brats_project.paths import PROJECT_RAW, PROJECT_PREPROCESSED
from brats_project.utilities.dataset_name_id_conversion import convert_id_to_dataset_name
...
def extract_fingerprint_dataset(dataset_id: int,
    ...
    if check_dataset_integrity:
        verify_dataset_integrity(join(PROJECT_RAW, dataset_name), num_processes)
...
def extract_fingerprints(dataset_ids: List[int], fingerprint_extractor_class_name: str = 'DatasetFingerprintExtractor',
                         ...
                         show_progress_bar: bool = True):
    """
    clean = False will not actually run this. This is useful when you want to reuse an existing fingerprint
    and avoid recomputing it.
    """
    fingerprint_extractor_class = recursive_find_python_class(join(brats_project.__path__[0], "experiment_planning"),
                                                              fingerprint_extractor_class_name,
                                                              current_module="brats_project.experiment_planning")
...
def plan_experiment_dataset(dataset_id: int,
                            ...
                            gpu_memory_target_in_gb: Optional[float] = None, preprocess_class_name: str = 'DefaultPreprocessor',
...
def plan_experiments(dataset_ids: List[int], experiment_planner_class_name: str = 'ExperimentPlanner',
    ...
    if experiment_planner_class_name == 'ExperimentPlanner':
        print("\n############################\n"
              "INFO: You are using the local default planner implementation for this BraTS project."
              "\n############################\n")
    experiment_planner = recursive_find_python_class(join(brats_project.__path__[0], "experiment_planning"),
...
def preprocess_dataset(dataset_id: int,
    ...
    # copy the gt to a folder in the PROJECT_PREPROCESSED so that we can do validation even if the raw data is no
    # longer there (useful for compute cluster where only the preprocessed data is available)
    from distutils.file_util import copy_file
    maybe_mkdir_p(join(PROJECT_PREPROCESSED, dataset_name, 'gt_segmentations'))
    dataset_json = load_json(join(PROJECT_RAW, dataset_name, 'dataset.json'))
    ...
    for k in dataset:
        copy_file(dataset[k]['label'],
                  join(PROJECT_PREPROCESSED, dataset_name, 'gt_segmentations', k + dataset_json['file_ending']),
                  update=True)
