...
from brats_project.inference.data_iterators import (
    PreprocessAdapterFromNpy,
    preprocessing_iterator_fromfiles,
    preprocessing_iterator_fromnpy,
)
from brats_project.inference.export_prediction import (
    export_prediction_from_logits,
    convert_predicted_logits_to_segmentation_with_correct_shape,
)
from brats_project.inference.sliding_window_prediction import (
    compute_gaussian,
...
from brats_project.utilities.utils import create_lists_from_splitted_dataset_folder
...
class nnUNetPredictor(object):
    ...
    def initialize_from_trained_model_folder(
        ...
        trainer_class = recursive_find_python_class(
            join(brats_project.__path__[0], "training"),
            trainer_name,
            "brats_project.training",
        )
        if trainer_class is None:
            raise RuntimeError(
                f"Unable to locate trainer class {trainer_name} in brats_project.training. "
                "Please place it under src/brats_project/training."
            )
        ...
        if (
            ("PROJECT_COMPILE" in os.environ.keys())
            and (os.environ["PROJECT_COMPILE"].lower() in ("true", "1", "t"))
    ...
    def manual_initialization(
        ...
        allow_compile = (
            allow_compile
            and ("PROJECT_COMPILE" in os.environ.keys())
            and (os.environ["PROJECT_COMPILE"].lower() in ("true", "1", "t"))
        )
        allow_compile = allow_compile and not is_compiled_module(self.network)
        if isinstance(self.network, DistributedDataParallel):
            allow_compile = allow_compile and not is_compiled_module(
                self.network.module
            )
        if allow_compile:
            print("Using torch.compile")
    ...
    def _manage_input_and_output_lists(
        ...
        output_folder_or_list_of_truncated_output_files: Optional[
            Union[str, List[str]]
        ],
    ...
    def get_data_iterator_from_raw_npy_data(
        ...
        segs_from_prev_stage_or_list_of_segs_from_prev_stage: Optional[
...
def predict_entry_point_modelfolder():
    ...
    parser = argparse.ArgumentParser(
        description="Use this to run inference from a manually specified trained model "
        "folder. This is useful when the project result directories "
        "(PROJECT_RESULTS) are not set."
    )
    parser.add_argument(
        "-i",
        type=str,
    ...
    parser.add_argument(
        ...
        "Use CUDA_VISIBLE_DEVICES=X brats_predict [...] instead!",
    )
    parser.add_argument(
...
def predict_entry_point():
    ...
    parser = argparse.ArgumentParser(
        description="Use this to run inference through the local BraTS project model "
        "registry. This is useful when trained models are stored in the "
    ...
    parser.add_argument(
        ...
        required=False,
        default="SegTrainer",
        help="Trainer class used for training. Default: SegTrainer",
    )
    parser.add_argument(
        "-c",
        type=str,
        required=True,
        help="Configuration that should be used for prediction. Config must be located "
    ...
    parser.add_argument(
        ...
        type=int,
        required=False,
        default=_getDefaultValue("BRATS_NPP", int, 3),
        help="Number of processes used for preprocessing. More is not always better. Beware of "
        "out-of-RAM issues. Default: 3",
    )
    parser.add_argument(
        ...
        type=int,
        required=False,
        default=_getDefaultValue("BRATS_NPS", int, 3),
        help="Number of processes used for segmentation export. More is not always better. Beware of "
        "out-of-RAM issues. Default: 3",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        required=False,
        default=1,
        help="Number of separate brats_predict calls that you will be making. Default: 1 (= this one "
    ...
    parser.add_argument(
        ...
        required=False,
        default=0,
        help="If multiple brats_predict calls exist, which one is this? IDs start with 0 can end with "
        "num_parts - 1. So when you submit 5 brats_predict calls you need to set -num_parts "
    ...
    ), "part_id must be smaller than num_parts. See brats_predict -h."
...
if __name__ == "__main__":
    ...
    from brats_project.paths import PROJECT_RESULTS, PROJECT_RAW
    ...
    # predictor.predict_from_files(join(PROJECT_RAW, 'Dataset003_Liver/imagesTs'),
    #                              join(PROJECT_RAW, 'Dataset003_Liver/imagesTs_predlowres'),
    #                              save_probabilities=False, overwrite=False,
    #                              num_processes_preprocessing=2, num_processes_segmentation_export=2,
    #                              folder_with_segs_from_prev_stage=None, num_parts=1, part_id=0)
    #
    # # predict a numpy array
    # from brats_project.imageio.simpleitk_reader_writer import SimpleITKIO
    #
    # img, props = SimpleITKIO().read_images([join(PROJECT_RAW, 'Dataset003_Liver/imagesTr/liver_63_0000.nii.gz')])
    ...
    ret = predictor.predict_from_files_sequential(
        [
            [
                "/media/isensee/raw_data/PROJECT_RAW/Dataset004_Hippocampus/imagesTs/hippocampus_002_0000.nii.gz"
            ],
            [
                "/media/isensee/raw_data/PROJECT_RAW/Dataset004_Hippocampus/imagesTs/hippocampus_005_0000.nii.gz"
            ],
        ],
